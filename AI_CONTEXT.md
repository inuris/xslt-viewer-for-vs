...
- **Zero = remove:** Dragging/typing to `0` doesn't set `width:0px` — it removes the `width`/`height` declaration entirely (and drops the whole `style=""` attribute if that was its only declaration).
