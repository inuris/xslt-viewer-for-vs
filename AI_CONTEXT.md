...
- **Badges:** Whenever an element is activated (click, or cursor-driven highlight), two badges (`W` / `H`) pin to the top-right corner of the anchor element (`activeEditEl`, keyed by `activeEditLine`).
- **Editing UI:** Clicking a badge opens a small floating popup with a range **slider** (`0` → the anchor's **parent** element's current width/height in px) plus a synced number input, prefilled from the element's current inline `style.width`/`height` (or its rendered pixel size if no inline value is set yet). Dragging/typing live-previews the value on the DOM optimistically; the edit commits (posts to the extension) on the slider/number `change` event (drag release, or Enter/blur on the number field), not on every intermediate tick. Escape reverts the optimistic DOM change and closes without committing.
  - **Popup follows the anchor, and scroll never force-closes it.** The popup is positioned off `editIconsBox`'s *current* rect (`positionEditInputBox()`), not a one-time snapshot of the badge's click position — a capture-phase `scroll` listener re-positions the icons/popup/handles on every scroll instead of hiding them. This mattered in practice: calling `.focus()` on the freshly-shown slider can itself trigger the browser's "scroll the focused element into view", which fired that same scroll listener; an earlier version called `hideEditInput()` there, so the popup closed ~100ms after opening on its own focus. Don't reintroduce a hide-on-scroll here.
- **Edge-drag handles (Photoshop-style):** Four invisible 6px hit-strips (`.xslt-edge-handle`, `ew-resize`/`ns-resize` cursor, highlight on hover) sit flush on the active element's left/right/top/bottom borders (`positionEdgeHandles()`). Mousedown on one starts a `document`-level `mousemove`/`mouseup` drag (`startEdgeDrag` → `onEdgeDragMove`/`onEdgeDragEnd`) that live-resizes width (left/right handles) or height (top/bottom handles) with a floating px label following the cursor, and commits through the same `commitStyleValue()` helper as the slider popup on mouseup. Left/top handles invert the sign (dragging left/up *increases* size, since the element's near edge is what's conceptually being pulled) — unlike the slider, drag is not clamped to the parent's size, only to `>= 0`.
- **Shared commit path:** Both the slider popup and the edge-drag handles funnel through `commitStyleValue(prop, line, px)` — `px <= 0` removes the declaration (see below), otherwise it writes `${px}px`. Keep new width/height entry points going through this helper rather than duplicating the 0-means-remove branch.
- **Zero = remove:** Reaching `0` (slider, typed number, or drag) doesn't set `width:0px` — it removes the `width`/`height` declaration entirely (and drops the whole `style=""` attribute if that was its only declaration).
...
- **Messages to Extension:** `jumpToCode`, `switchFile`, `exportPdf`, `exportImage`, `replaceImage`, `jumpToImage`, `toggleLock` (`{ locked: boolean }`), `editElementStyle` (`{ line, prop: 'width'|'height', value }`, forwarded from the iframe — see §9; empty `value` means "remove the declaration"). Replace panel also sends `replaceImageReady`, `replaceImagePickFile`, `replaceImageApply`, `replaceImageDelete`, `replaceImageCancel`.
