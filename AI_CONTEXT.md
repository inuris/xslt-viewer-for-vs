...
- **Apply:** Commit posts `{ command: 'editElementStyle', line, prop: 'width'|'height', value }` (an empty `value` means "remove") up through the webview shell to the extension, which calls `styleEdit.ts`'s `applyInlineStyleEdit()` against `activeXslt` and, on success, calls `runUpdate(message.line)` to re-render from the now-edited source and re-activate the same line post-reload (width/height are the only props that still trigger this reload — see §10's last gotcha for why font-weight/color don't).
...
### 10. Quick-Edit Controls (Bold + Color, Preview Pane Toolbar)
- **Location (static, not floating):** A `B` toggle button (`#qt-bold-btn`) and a native `<input type="color">` (`#qt-color-input`) live as ordinary buttons in the *outer* preview toolbar (`getWebviewShell()`, between the Lock toggle and the Zoom dropdown — same row as Export PDF/Lock/Zoom/Images), disabled whenever no preview element is active. This landed here after several rounds of a *floating* toolbar anchored inside the iframe (positioned above/below the active element, à la Word's quick-edit popup) turned out fundamentally unreliable: focusing its color input could trigger the browser's "scroll focused element into view," which fired the iframe's scroll listener, which repositioned the floating toolbar mid-interaction — and Chromium/Electron auto-dismisses a native color popup the instant its anchor visibly moves. Clamping, guarding, skipping reloads, etc. all narrowed the failure window but never eliminated it (reliably broke by the 2nd element activated in a session). Moving the controls to the static outer toolbar removes the failure mode structurally: a fixed button has no position to lose, so there is nothing left for a scroll/reposition to race against, and — since these controls now live in a completely different document than the preview iframe — clicking them never generates a `click` event inside the iframe's `document` at all, which also eliminates the entire earlier class of "stray click after popup closes lands on `<body>` and deactivates" bugs (same family as §9's post-edge-drag stray click) for these two controls specifically.
- **State bridge (iframe owns activation, outer toolbar just relays intent):** The iframe still owns `activeEditEl`/`activeEditLine` (§9) — nothing about *which* element is active moved. Whenever activation changes (`activateElements()` / `clearPreviewLineHighlight()`), the iframe calls `reportActiveElementState()`, which posts `{ command: 'activeElementState', active, bold, color }` (hex, via `rgbStringToHex()`) up to `window.parent`. The outer shell's iframe-message listener (the one that already forwards `jumpToCode`/`editElementStyle` to the extension) handles `activeElementState` locally — `setQuickToolbarActive(active, bold, colorHex)` enables/disables both controls and reflects the Bold-active state / current color, no forwarding to the extension needed. Conversely, clicking the outer Bold button calls `qtToggleBold()`, and the color input's `input`/`change` listeners post `{ command: 'qtSetColor', mode: 'preview'|'commit', value }` — both go *down* into the iframe via `frame.contentWindow.postMessage(...)`. The iframe's own `message` listener applies them against `activeEditEl` (`mode: 'preview'` just live-previews the color without committing, matching a color picker's in-progress drag; `mode: 'commit'`/Bold both call `commitGenericStyle(prop, value)`, which applies the style locally, posts `editElementStyle` to the extension, and re-reports state).
- **Bold semantics:** `isBold(el)` reads `getComputedStyle(el).fontWeight` (numeric ≥ 600, or `'bold'`/`'bolder'`) so it reflects inherited/class-based bold too, not just an existing inline style. Toggling off removes the declaration (same empty-value-means-remove convention as §9) rather than writing `font-weight: normal`.
- **Message path / persistence:** Both reuse `commitGenericStyle(prop, value)` → `{ command: 'editElementStyle', line, prop: 'font-weight'|'color', value }` up to `extension.ts`, whose handler and `styleEdit.ts`'s `applyInlineStyleEdit()` accept the `StyleProp` union (`'width'|'height'|'font-weight'|'color'`) — the underlying attribute-patching regex logic was already prop-name-generic. Unlike width/height, `font-weight`/`color` commits do **not** trigger `runUpdate()` (`needsReload = prop === 'width' || prop === 'height'` in the handler) — the DOM is already showing the change live by the time the message is sent, so a full transform+iframe-reload round trip was pure risk (see the git history around this section for the specific races it caused) with no benefit; the persisted XSLT source and the live preview simply stay in sync silently in the background.
...
- **Toolbar** (`#toolbar`): Export PDF button | Lock toggle (`🔓 Lock` / `🔒 Locked`) | Bold toggle + text-color input (`#qt-bold-btn`/`#qt-color-input`, disabled unless a preview element is active — see §10) | Zoom dropdown (25/50/75/100%) | Images sidebar toggle. The Lock button sits immediately left of the Bold/color controls and, when on, prevents the preview from auto-switching to a different XML file's pair (see §2). The zoom dropdown is initialized from `xslt-viewer.previewZoom`, and changes are sent back via `setPreviewZoom` to persist the last choice.
