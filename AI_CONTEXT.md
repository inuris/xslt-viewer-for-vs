...
- **Messages to Extension:** `jumpToCode`, `switchFile`, `exportPdf`, `editImage`, `jumpToImage`, `toggleLock` (`{ locked: boolean }`), `editElementStyle` (`{ line, prop: 'width'|'height', value }`, forwarded from the iframe — see §9; empty `value` means "remove the declaration"). The Edit Image panel also sends `editImageReady`, `editImagePickFile`, `editImageSave`, `editImagePreview`, `editImagePreviewReset`, `editImageApply`, `editImageDelete`, `editImageCancel`.
