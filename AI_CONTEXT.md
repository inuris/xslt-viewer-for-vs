...
- **Gotcha (the real fix: font-weight/color don't reload the iframe at all anymore):** Several rounds of Bold/color activation bugs (webview clicks leaving `vscode.window.activeTextEditor` stale so `highlightLine` silently dropped; a redundant 500ms-debounced `runUpdate()` from `workspace.onDidChangeTextDocument`/`triggerAutoUpdate` clobbering a just-restored activation moments later; the async transform-and-reload round trip landing mid-click on a *different* element and killing that click's native-picker-open default action) all traced back to the same root cause: every `editElementStyle` commit did a *full* iframe reload (`frame.srcdoc = ...`), which wipes all script state (`activeEditEl`, highlight, edge handles, quick toolbar) and requires racing an async transform+re-render to restore it. For `font-weight`/`color` that reload was never actually necessary — the iframe script already applies the change live to `activeEditEl` (`activeEditEl.style.setProperty(...)`/`.style.color = ...`) before the message is even sent, so the preview is already correct; the reload existed only to defensively re-sync from the persisted source, and re-syncing something that's already in sync is exactly what kept racing with the *next* interaction. `extension.ts`'s `editElementStyle` handler now only calls `runUpdate(message.line)` for `width`/`height` (`needsReload = prop === 'width' || prop === 'height'`); `font-weight`/`color` still persist to the XSLT source via `applyInlineStyleEdit()` (and still clear the stale `updateTimeout`, since that race is still real for width/height), but skip the reload — so there's no iframe replacement to race against a subsequent click on a different element's toolbar, and no activation to restore in the first place. Earlier partial fixes (the `runUpdate(forceHighlightLine)` override, clearing `updateTimeout` on commit) are still in effect and still needed for width/height's reload path.
- **Gotcha (color picker closing synthesizes a stray click, same story as edge-drag mouseup):** Independent of the reload issue above — dismissing a native `<input type="color">` popup (picking a color, Escape, or clicking away) synthesizes a `click` on whatever's under the pointer at that moment, not the color input itself, since the OS/browser color popup renders at arbitrary screen coordinates far from the small toolbar swatch. That stray click landed in the main `document` click handler's "else" branch (no `data-source-line` under it, typically `<body>`), deactivating the just-recolored element — structurally the same bug as §9's post-edge-drag stray click. Fixed by setting `justDragged` from `qtColorInput`'s `change` listener so the very next click is swallowed. A `blur`-based version was also tried, to additionally cover a same-value dismissal (Escape, or reopening/closing without picking) that doesn't fire `change` — reverted after it made the *opening* click itself intermittently flaky: `blur` timing relative to the native popup's actual open/close isn't reliable across Chromium/Electron builds, and appeared to fire early enough in some runs that its `justDragged` got consumed by an unrelated click before the real dismissal happened. `change`-only is narrower (dismissing without changing anything can still lose activation) but doesn't destabilize the common "pick a color" path.
- **Gotcha (focusing the color input can itself trigger a browser auto-scroll that closes its own popup — same lesson as the old W/H popup, recurring):** Reliably reproduced from the *second* activation onward (first element fine, every one after: picker flashes open then immediately closes, activation lost to `<body>`). Cause: clicking `qtColorInput` gives it focus as part of opening its native picker; if the input (a `position:fixed` toolbar element tracking `activeEditEl`'s bounding rect) isn't fully inside the current viewport at that instant, the browser's default "scroll the newly-focused element into view" kicks in — an iframe-internal scroll, which fires the capture-phase `document` `scroll` listener, which called `positionQuickToolbar()` and re-set the toolbar's `left`/`top`. Chromium/Electron treats that as the popup's anchor moving out from under it and auto-dismisses the popup — this is the *exact same root cause* documented in §9's history for the now-removed W/H badge+slider popup ("scroll must only reposition, never hide"), just resurfacing through a different mechanism (native popup dismissal instead of our own `display:none`) because repositioning alone, not just hiding, is enough to trigger it for a native control. Fixed two ways in `positionQuickToolbar()`: (1) it now clamps `left`/`top` fully inside `window.innerWidth`/`innerHeight` on both axes (previously only clamped the left edge and the top-flip, not the right/bottom), so the toolbar is essentially always already fully visible and focusing it shouldn't need a browser auto-scroll in the first place; (2) as a backstop, it now no-ops entirely whenever `document.activeElement === qtColorInput` — while the color input holds focus (its picker likely open), nothing repositions it even if a scroll does happen for some other reason, so its on-screen position never changes underneath the popup.
