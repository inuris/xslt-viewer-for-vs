...
- **Trigger:** Whenever the purple "active element" highlight (`.xslt-preview-line-highlight`, §3 "Click-to-Jump" / reverse cursor-sync) is applied — i.e. after a preview click-to-jump roundtrip OR when the XSLT cursor lands on a mapped line — two small badges (`W` / `H`) appear pinned to the top-right corner of the first highlighted element. They're keyed to `fallbackLine` (the line that actually carries `data-source-line`), not the raw requested line, so an edit always targets the tag the icons are anchored to.
- **Editing:** Clicking a badge opens a small floating text input in the preview, prefilled with the element's current inline `style.width`/`height` (or its rendered pixel size if no inline value is set yet). Enter commits, Esc/blur-without-change cancels. A bare number is treated as `px`; any other unit (`%`, `em`, ...) is passed through as typed.
- **Apply:** Commit sets the value on the live preview element immediately (optimistic), then posts `{ command: 'editElementStyle', line, prop: 'width'|'height', value }` up through the webview shell to the extension, which calls `styleEdit.ts`'s `applyInlineStyleEdit()` against `activeXslt` and, on success, calls `runUpdate()` to re-render from the now-edited source (so the preview reflects the actual persisted change, not just the optimistic DOM tweak).
- **Known limits:** Only edits literal/static XSLT output tags (same constraint as click-to-jump/`instrumentXslt`). If two output tags start on the same source line, the edit targets the first one in source order. Always writes an inline `style` declaration, which by CSS specificity overrides any class-based or legacy `width=""`/`height=""` attribute sizing regardless of where the original value came from.
...
- **Messages to Extension:** `jumpToCode`, `switchFile`, `exportPdf`, `exportImage`, `replaceImage`, `jumpToImage`, `toggleLock` (`{ locked: boolean }`), `editElementStyle` (`{ line, prop: 'width'|'height', value }`, forwarded from the iframe — see §9). Replace panel also sends `replaceImageReady`, `replaceImagePickFile`, `replaceImageApply`, `replaceImageDelete`, `replaceImageCancel`.
